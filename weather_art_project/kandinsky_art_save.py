"""
康定斯基风格空气质量抽象艺术 - 保存版本
Kandinsky-Style Air Quality Abstract Art - Save Version

将空气质量数据转换为康定斯基风格的抽象几何图形并保存为图片
"""
...
class KandinskyAirArtSave:
    ...
    def save_frames(self, num_frames=10):
        """保存多帧图片"""
        print("开始生成康定斯基风格抽象艺术图片...")
        ...
        # 创建输出目录
        output_dir = "kandinsky_frames"
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        
        # 生成并保存多帧
        for frame in range(num_frames):
            print(f"生成第 {frame + 1}/{num_frames} 帧...")
            
            # 创建可视化
            self.create_abstract_visualization(frame)
            
            # 保存图片
            filename = f"{output_dir}/kandinsky_frame_{frame:03d}.png"
            plt.savefig(filename, dpi=300, bbox_inches='tight', facecolor='#e5dbc3')
            print(f"已保存: {filename}")
        
        print(f"完成！共生成 {num_frames} 帧图片，保存在 {output_dir} 目录中")
    
    def save_single_frame(self, frame=0):
        """保存单帧图片"""
        print("生成康定斯基风格抽象艺术图片...")
        ...
        # 保存图片
        filename = "kandinsky_abstract_art.png"
        plt.savefig(filename, dpi=300, bbox_inches='tight', facecolor='#e5dbc3')
        print(f"已保存: {filename}")

def main():
    """主函数"""
    print("=== 康定斯基风格空气质量抽象艺术 - 保存版本 ===")
    print("将空气质量数据转换为抽象几何图形并保存为图片")
    print()
    
    # 创建抽象艺术生成器
    artist = KandinskyAirArtSave()
    
    # 保存单帧图片
    artist.save_single_frame()
    
    # 可选：保存多帧图片
    # artist.save_frames(num_frames=5)
